...
export default async function Page({ params }: { params: PageParams }) {
  ...
  const createdTime = new Date(expertItem.created_time).toLocaleDateString('ko-KR', {
    year: 'numeric',
    month: '2-digit',
  })
  const title = (expertItem.properties.title as any).title[0].plain_text
  const subTitle = (expertItem.properties.subtitle as any).rich_text[0]?.plain_text || 'No subtitle available'
